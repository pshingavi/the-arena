...
export default function ArenaClient() {
  ...
  return (
    <div className="min-h-screen bg-arena-bg flex flex-col">
      ...
      {/* ── Top bar ────────────────────────────────────────────────────────── */}
      <header className="border-b border-arena-border px-6 py-3 flex items-center gap-4 flex-shrink-0 bg-arena-card/50 backdrop-blur-sm">
        <button onClick={resetDebate} className="text-arena-muted hover:text-white transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </button>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            ...
            <span className="text-xs font-bold text-white uppercase tracking-wider">The Arena</span>
            ...
            {isWaitingForVideo && !activeTurnVideo && (
              <span className="flex items-center gap-1 text-xs text-yellow-400/80 font-medium">
                <span className="w-1.5 h-1.5 rounded-full bg-yellow-400/80 animate-pulse inline-block" />
                Preparing avatar...
              </span>
            )}
          </div>
          <div className="text-xs text-arena-muted truncate">{finalTopic}</div>
        </div>

        <div className="hidden md:flex items-center gap-2 text-xs text-arena-muted">
          Turn {Math.min(turnNumber + 1, DEBATE_SEQUENCE.length)} / {DEBATE_SEQUENCE.length}
        </div>

        <button onClick={toggleMusic}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-arena-border text-xs text-arena-muted hover:text-white hover:border-white/30 transition-all">
